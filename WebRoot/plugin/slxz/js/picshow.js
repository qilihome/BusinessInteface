...
FCAPP.HOUSE.PICSHOW = {
    ...
    loadData: function() {
        ...
        datafile += 'picdatas.js?';
        $.ajax({
            url: 'data/' + datafile + Math.random(),
            dataType: 'jsonp'
        });
    },
    ...
    renderPics: function(data) {
        ...
        R.scrollTips.html($.template(R.template2, {
            data: R.scrollTitle.slice(0, 3),
            idx: 0
        }));
    ...
    calcWidth: function(part, dataIdx) {
        ...
        for (var i in part) {
            ...
            for (var j = 0, jl = data.length; j < jl; j++) {
                if (data[j].type == 'img') {
                    ...
                    str = data[j].content.replace(/[a-z0-9]+/gi, '');
                    len = str.length + Math.ceil((data[j].content.length - str.length) / 2);
                    cw = Math.ceil(len * 150 / 96);
                    data[j].width = cw;
                    textIdx = i;
                    textLoc = j;
        ...
        cw = Math.abs(width[titleIdx] - width[textIdx]);
        R.reduceSize += width[titleIdx] + width[textIdx] + 10;
        if (width[titleIdx] > width[textIdx]) {
            R.reduceSize += cw + 11;
        }
        if (cw > 10) {
            width[textIdx] += cw + 12;
            part[textIdx][textLoc].width += cw - 12;
        } else {
            width[textIdx] += cw + 10;
        }
