...
FCAPP.HOUSE.List = {RUNTIME: {LI: {}},init: function() {
        List.initElements();
        List.loadListData();
        var id = '';
      ...
      /*  var R = List.RUNTIME;
       ...
       /* window.showRooms = List.showRooms;
        ...
        datafile = datafile.replace(/[<>\'\"\/\\]+/gi, '');
        datafile += 'rooms.js?';
        $.ajax({url: '/fangchan/static/' + datafile + Math.random(),dataType: 'jsonp'});*/
    },showRooms: function(res) {
     /*   var R = List.RUNTIME;
        FCAPP.Common.hideLoading();
        var data = res.rooms, content = '', categories = List.sortCategory(data);
        FCAPP.Common.loadImg(res.banner, 'bannerPic', function(img) {
            img.width = 720;
            img.height = 130;
            img.id = img.idx;
            R.actTop.show();
        });
        content = $.template(R.template, {data: categories});
        setTimeout(function() {
            R.container.html(content);
        }, 30);*/
    },sortCategory: function(rooms) {
        var tmp = [], category = {}, newCategory = [], key, other, i, defaultDesc = '其他户型';
        ...
        for (i in category) {
            tmp.push({key: i,len: category[i].length});
        }
        ...
        for (i = 0, il = tmp.length; i < il; i++) {
            key = tmp[i].key;
            if (key !== defaultDesc) {
                newCategory.push({key: key,rooms: category[key]});
            }
        }
        if (category[defaultDesc] && category[defaultDesc].length > 0) {
            newCategory.push({key: defaultDesc,rooms: category[defaultDesc]});
        }
        tmp = null;
        category = null;
        return newCategory;
    },showDetail: function(openUrl) {
		 //location.href = openUrl;
       // FCAPP.Common.jumpTo('picroll.html', {houseid: id,'#wechat_webview_type': 1}, true);
    },show3D: function(houseid) {
        //FCAPP.Common.jumpTo('3dfull.html', {houseid: houseid,'#wechat_webview_type': 1}, true);
    },switchList: function(obj, len, idx) {
        var R = List.RUNTIME.LI, box = obj.parentNode;
        if (!R['boxLiBox' + idx]) {
        ...
        var R = List.RUNTIME.LI, height = 0, cls = '';
        if (!R['boxLi' + id]) {
            R['boxLi' + id] = $('#boxLi' + id);
...
var HOUSE = FCAPP.HOUSE, List = HOUSE.List;
